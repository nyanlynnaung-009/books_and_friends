...
import { BookOpen, PlusSquare, LibraryBig, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-gradient-to-r from-rose-500 to-rose-600 text-white px-5 py-2.5 rounded-xl hover:from-rose-600 hover:to-rose-700 shadow-sm hover:shadow-md transition-all font-medium active:scale-95"
        >
          <PlusSquare className="w-5 h-5" />
          <span className="hidden sm:inline">{t('upload_book')}</span>
        </button>
      </motion.div>
      ...
      {loading ? (
        ...
        <motion.div 
          ...
          className="text-stone-500 dark:text-stone-400 text-center py-20 bg-white dark:bg-stone-800 rounded-3xl border border-stone-200 dark:border-stone-700 border-dashed flex flex-col items-center justify-center"
        >
          <BookOpen className="w-16 h-16 mb-4 opacity-20" />
          <p className="text-lg">{t('no_books_library')}</p>
        </motion.div>
      ) : (
        <motion.div 
